...
def train_model(algorithm, data_dir, dataset_name, root_config_file, epochs=250, batch_size=2, train_val_split=0.9,
                base_workspace="/media/tiger/Disk0/xxu/netstore/workspace/", save_checkpoint_every_n_epochs=50, **kwargs):
    kwargs["root_config_file"] = root_config_file

    special_name = 'vessel3d'

    working_dir = get_writing_path(Path(base_workspace).expanduser() / (algorithm + "_" + special_name),
                                   root_config_file)
